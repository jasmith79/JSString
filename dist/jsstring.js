...
const printfFormat = (c, arg) => {
    ...
    switch (c) {
        ...
        case 'd':
            if (Number.isNaN(x))
                throw new TypeError(`Non numeric argument ${arg} to %${c}`);
            return (x | 0).toString();
        case 'b':
            if (Number.isNaN(x))
                throw new TypeError(`Non numeric argument ${arg} to %${c}`);
...
export const padLeft = (n, str, char = ' ') => str.length > n
    ? str
    : char.repeat(n - str.length) + str;
export const padRight = (n, str, char = ' ') => str.length > n
    ? str
...
//# sourceMappingURL=jsstring.js.map